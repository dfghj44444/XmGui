...
namespace XM
{
	...
	public:
		...
		inline bool const		getVisible() const		  { return m_bVisible; };

		inline void				setUseKeyboard(bool bFlag){	m_bUseKeyboard = bFlag; };
		inline bool				getUseKeyboard()const{ return m_bUseKeyboard; };
		inline void				setUseMouse(bool bFlag){ m_bUseMouse = bFlag; };
		inline bool				getUseMouse()const{ return m_bUseMouse; };

		inline int   			getID() const		{ return m_nID; };
		virtual void			setID(int iId);
		inline int				getSuperID() const	{ return m_nSuperID; };
		virtual void			setSuperID(int nId);

		// position, size
		void				setParentPos(const D3DXVECTOR3& vPos){ 
			this->setParentPos(vPos.x,vPos.y,vPos.z); 
		};
		...
		// message handler
		virtual void			onFocusIn() { m_bHasFocus = true; updateControlState(); };
		virtual void			onFocusOut(){ m_bHasFocus = false; updateControlState(); };
		virtual void			onMouseEnter(){ m_bMouseOver = true; updateControlState(); };
		virtual void			onMouseLeave(){ m_bMouseOver = false; updateControlState();};
